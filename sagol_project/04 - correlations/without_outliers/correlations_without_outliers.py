...
ff_fdr_matrix.to_csv(out_dir / "p_fdr_feature_feature_matrix_without_outliers.csv")

# feature–ARI
ari_fdr = all_tests_df.query("Type == 'feature-ARI'")[["Feature_1", "p_fdr"]]
ari_fdr.to_csv(out_dir / "p_fdr_feature_ARI_without_outliers.csv", index=False)

# feature–SCARED
sc_fdr = all_tests_df.query("Type == 'feature-SCARED'")[["Feature_1", "p_fdr"]]
sc_fdr.to_csv(out_dir / "p_fdr_feature_SCARED_without_outliers.csv", index=False)
...
sig_df.to_csv(out_dir / "all_significant_correlations_without_outliers.csv", index=False)

# per-type significant
for t in ["feature-feature", "feature-ARI", "feature-SCARED"]:
    df_sub = sig_df.query("Type == @t")
    fn = f"significant_{t.replace('-', '_')}_without_outliers.csv"
    df_sub.to_csv(out_dir / fn, index=False)

# full matrices
corr_ff.to_csv(out_dir / "correlation_matrix_without_outliers.csv")

# feature–ARI and SCARED (with FDR)
sig_df.query("Type=='feature-ARI'")[['Feature_1','r','p_fdr']].to_csv(
    out_dir / "features_vs_ARI_without_outliers.csv", index=False)

sig_df.query("Type=='feature-SCARED'")[['Feature_1','r','p_fdr']].to_csv(
    out_dir / "features_vs_SCARED_without_outliers.csv", index=False)