...
for mode in feature_modes:
    ...
    model_dir = Path(f"results/{mode}_model")
    ...
    for train_idx, test_idx in loo.split(X_train):
        ...
        if X_tr_nofeatures.shape[1] == 0:
            print(f"No features selected for mode '{mode}'. Skipping modeling.")
            break
    ...
    if loocv_df.empty or not any(f"y_pred_{alpha}" in loocv_df.columns for alpha in alphas):
        print(f"No LOOCV predictions for mode '{mode}'. Skipping metrics and test evaluation.")
        continue
