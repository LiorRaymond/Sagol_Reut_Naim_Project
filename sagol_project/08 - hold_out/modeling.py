...
# Load train/test indices from X_train_final and X_test_final
X_train = pd.read_csv("results/X_train_final.csv")
X_test = pd.read_csv("results/X_test_final.csv")

# Get y_train and y_test using the indices from the original data based on IDs
...
for train_idx, test_idx in loo.split(X_train):
    ...
    X_tr_imp = imputer.fit_transform(X_tr.drop(columns=["ID"]))
    X_te_imp = imputer.transform(X_te.drop(columns=["ID"]))
    ...
    for alpha in alphas:
        ...
        preds_alpha.append(pred)
    loocv_records.append(rec)

# Save LOOCV results
loocv_df = pd.DataFrame(loocv_records)
loocv_df.to_csv("results/holdout_LOOCV_results.txt", sep="\t", index=False)
...
metrics_df.to_csv("results/holdout_overall_metrics.txt", sep="\t", index=False)

# Save classification reports for each alpha
with open("results/holdout_classification_reports.txt", "w") as f:
    for alpha in alphas:
        preds = loocv_df[f"y_pred_{alpha}"]
...
test_results.to_csv("results/holdout_test_predictions.txt", sep="\t", index=False)

# Save test summary metrics
with open("results/holdout_test_summary.txt", "w") as f:
